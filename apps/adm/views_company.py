...
class CompanyCorpView(LoginRequiredMixin,View):
    """
    入驻公司管理
    """
    ...
    def get(self, request):
        ...
        return render(request, 'adm/company/corp.html', ret)


class CompanyCorpListView(LoginRequiredMixin, View):
    """
    入驻公司管理
    """
    ...
    def get(self, request):
        ...
        ret = dict(data=list(Corp.objects.filter(**filters).values(*fields)))
        return HttpResponse(json.dumps(ret, cls=DjangoJSONEncoder), content_type='application/json')


class CompanyCorpCreateView(LoginRequiredMixin,View):
    """
    创建入驻公司
    """
    def get(self, request):
